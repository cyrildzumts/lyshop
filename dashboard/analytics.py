...
def report_products(year=timezone.now().year):
    ...
    data = []
    product_type_report = Product.objects.filter(quantity__gt=0).objects.values(type_name=F'product_type__display_name').annotate(count=Count('product_type')).order_by()
