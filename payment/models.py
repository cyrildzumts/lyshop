...
class Payment(models.Model):
    ...
    bank_name = models.CharField(max_length=64, blank=True, null=True)
    bank_account_number = models.CharField(max_length=64, blank=True, null=True)
    payment_status = models.IntegerField( default=PaymentConf.PAYMENT_SUBMITTED, blank=False, null=False, choices=PaymentConf.PAYMENT_STATUS)
    declined_reason = models.IntegerField( blank=True, null=True, choices=PaymentConf.DECLINED_REASON)
    policy = models.ForeignKey(PaymentPolicy, blank=True, null=True, on_delete=models.SET_NULL)
    monthly_limit = models.IntegerField(blank=False)
    payment_mode = models.IntegerField( default=PaymentConf.PAYMENT_PAY, blank=False, null=False, choices=PaymentConf.PAYMENT_MODE)
